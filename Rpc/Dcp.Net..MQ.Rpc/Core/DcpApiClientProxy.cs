...
namespace Dcp.Net.MQ.Rpc.Core
{
   ...
   /// </summary>
    static class DcpApiClientProxy
    {
        ...
        /// 接口对应的代理类型的构造器缓存
        /// </summary>
        private static readonly ConcurrentCache<Type, System.Reflection.ConstructorInfo> proxyTypeCtorCache = new ConcurrentCache<Type, System.Reflection.ConstructorInfo>();
        ...
        public static object CreateInstance(Type interfaceType, IApiInterceptor interceptor)
        {
            ...
            {
                var assemblyName = typeof(DcpApiClientProxy).GetTypeInfo().Assembly.GetName();
        ...
        private static Type FindProxyType(this Type interfaceType)
        {
            ...
            {
                var rtnType=allTypes.FirstOrDefault(item => IsProxyType(interfaceType, item));
                return rtnType;
            }
            else
            {
