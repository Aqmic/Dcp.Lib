...
namespace Dcp.Net.MQ.Rpc.Handler
{
    ...
    public class PathQueryAttribute : Attribute, IApiParameterAttribute
    {
        ...
        public async Task BeforeRequestAsync(ApiActionContext context, ApiParameterDescriptor parameter)
        {
           ...
           // var options = context.ApiConfig.FormatOptions.CloneChange(this.DateTimeFormat);
          //  var keyValues = context.ApiConfig.KeyValueFormatter.Serialize(parameter, options);

            //context.RequestMessage.RequestUri = this.UsePathQuery(ur(), null);
