...
namespace Dcp.Net.Modules
{
    ...
    /// </summary>
    public abstract class DcpModule
    {
        /// <summary>
        ///     依赖注入管理器
        /// </summary>
        ...
        ///     初始配置
        /// </summary>
        protected internal IDcpStartupConfiguration Configuration { get; internal set; }
        ...
        ///     构造函数
        /// </summary>
        protected DcpModule() { Logger = NullLogger.Instance; }
        ...
        public static bool IsDcpModule(Type type) => type.GetTypeInfo().IsClass && !type.GetTypeInfo().IsAbstract && !type.GetTypeInfo().IsGenericType && typeof(DcpModule).IsAssignableFrom(type);
        ...
        {
            if (!IsDcpModule(moduleType)) throw new DcpInitException("此类型不是一个有效的模块: " + moduleType.AssemblyQualifiedName);
        ...
        public static List<Type> FindDependedModuleTypesRecursively(Type moduleType)
        {
            ...
            if (!list.Contains(typeof(DcpKernelModule))) { list.Add(typeof(DcpKernelModule));}
            return list;
        }
        ...
        {
            if (!IsDcpModule(module)) throw new DcpInitException("此类型不是一个有效的模块: " + module.AssemblyQualifiedName);
